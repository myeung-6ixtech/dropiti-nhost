...
export default async function paymentMethodsCreate(req: Request, res: Response): Promise<void> {
  if (req.method !== "POST") {
    fail(res, "Method not allowed", 405);
    return;
  }
  await withAdminAirwallex(req, res, {
    tag: "admin/payment-methods/create",
    rateKey: "airwallex:payment-methods",
    handler: async () =>
      airwallex.paymentMethods.create(
