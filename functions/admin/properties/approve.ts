...
const APPROVE = `
  mutation ApproveProperty($propertyUuid: uuid!) {
    update_real_estate_property_listing(
      where: { property_uuid: { _eq: $propertyUuid } }
      _set: { status: "published", completion_percentage: 100, updated_at: "now()" }
    ) {
      affected_rows
      returning { property_uuid title status }
    }
  }
`;

export default async function approveProperty(req: Request, res: Response): Promise<void> {
  try {
    ...
    await logAdminAction(payload, "property.approve", "property", body.propertyUuid, { notes: body.notes }, req);
    ok(res, { property: result.data?.update_real_estate_property_listing?.returning?.[0] ?? null });
  } catch (e) {
    console.error("[admin/properties/approve]", e);
