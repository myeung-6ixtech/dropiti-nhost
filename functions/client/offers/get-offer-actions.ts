...
export default async function getOfferActions(req: Request, res: Response): Promise<void> {
  try {
    ...
    ok(res, { actions: basicAvailableActions(offer, userId) });
  } catch (error) {
    console.error("[client/offers/get-offer-actions]", error);
